...
For this theme, `.md` and `.mdx` is supported by default.
...
If you want to use remote image for `heroImage`, please add `inferSize: true` or specified `width` and `height` to the object `heroImage`. Example:
...
heroImage:
  { src: './thumbnail.jpg', width: 980, height: 551 }
```
...
<Aside type='tip'>
This theme has a quick script to create a new article for you. Just run `bun pure new <post-slug>` in your terminal.
</Aside>
...
And in this theme, luckily you can use `BaseLayout.astro` as your layout component.
...
While `.astro` pages have some flexible features and be friendly to inline / module scripts, `.md` and `.mdx` pages are more suitable for content-focused pages.
